...
We can think of CLMs as a series of logistic regressions. If our observations $Y$ fall into ordinal categories labeled 1, 2, 3, 4, and 5 (as with a Likert-type scale), our model is as follows:

$$
P(Y =1) = \sigma(\theta_1 + x\beta) \\
...
ndata <- 1000
cont <- 1.65 + rnorm(1000, sd = 0.07)
categ <- runif(1000) < 0.6 + 0
latent_var <- 0.6*cont + 0.5*categ + 0.8*rnorm(100, sd = 0.1)
data <- data.frame(
  height = cont,
  categ = factor(categ,
                 levels = c(T, F),
...
Let's add another independent predictor:

## Plotting CLMs: continuous predictors

## Plotting CLMs: categorical predictors?
