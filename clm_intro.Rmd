...
ndata <- 1000
cont <- rnorm(ndata, sd = 0.07)
categ_effs <- c('A' = 0.5, 'B' = 0.6)
categ <- sample(names(categ_effs), ndata, replace = T)
latent_var <- 1.65 + 0.5*cont + categ_effs[categ] + rnorm(100, sd = 0.05)
data <- data.frame(
  height = cont,
  heels = factor(categ,
                     levels = c('A', 'B'),
...
Suppose the proportional odds assumption was violated for the `heels` term. In this case, we could fit a **partial proportional odds** model, where $\beta$ is allowed to vary across categories for `heels`, making it a "nominal effect". This is done using the `nominal` argument to `clm`:
...
So far, we've been working with **flexible** thresholds. This means the only restriction we impose on the $\theta$ intercepts is that they're monotonically increasing: i.e., $\theta_1 < \theta_2 < \theta_3 < \ldots$

[Actually is this necessary? Can't find references for when you'd want to impose this, and even the manual is vague about this]
