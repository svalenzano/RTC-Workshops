...
print(coef(mod))
```
For simplicity and interpretability, CLMs assume that the effect of the predictor variable is consistent across all cumulative logits. This means that the model imposes the same slope across different thresholds, also known as the proportional odds (PO) assumption. Because of this assumption, there is only one set of odds ratios in an ordinal model - what changes across categories are the thresholds rather than the predictor effects. 

```{r} 
# To test whether the PO assumption holds, we can use the nominal_test() function. This test checks whether the effect of the predictor varies across different response levels. If the test yields a significant p-value, it suggests that the effect of the predictor differs across the response categories, thereby violating the PO assumption.

nominal_test(mod) 

# If the PO assumption is violated you can run a Partial Proportional Odds Model to relax this assumption. This model allows for certain predictors to be treated as nominal (i.e., varying across thresholds), while still assuming proportional odds for other predictors. 

# mod.nom <- clm(agreement ~ height + categ, nominal = ~ categ, data = data) 

### Can delete this - If you want to compare the two models (the original CLM and the Partial Proportional Odds Model), you can use a likelihood ratio test to assess model fit. 
# anova(mod, mod.nom) 
```
## Plotting CLMs
...
To fit a CLMM to this data, we just need to add the term `(1|ptpt)` to specify that we want a random intercept for each participant. This is the same syntax as the `lmer` function from the `lme4` package:
