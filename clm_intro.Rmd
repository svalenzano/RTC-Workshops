...
3.  What if one of the predictors is ordinal?
...
$$
P(Y \leq 1) = \sigma\left(\theta_1 + x\beta\right) \\
P(Y \leq 2) = \sigma\left(\theta_2 + x\beta\right) \\
P(Y \leq 3) = \sigma\left(\theta_3 + x\beta\right) \\
P(Y \leq 4) = \sigma\left(\theta_4 + x\beta\right) \\
P(Y \leq 5) = 1 \\
$$
...
So far, we've just used a simple call to `plot()`, but this doesn't give us much control over how the plot is rendered. Instead, we can convert the output of `ggpredict` to a `data.frame` object that we can then pass to `ggplot`:
...
Another idea (very experimental)
...
The equations underlying CLMMs are the same as those underlying CLMs, except that random effects are incorporated. In our case:

$$
P(Y \leq 1) = \sigma\left( \theta_1 + \beta x + u\right) \\
\text{etc.}
$$

where $u$ is a random intercept. This means that the thresholds are shared across levels of your clustering variable.
...
To fit a CLMM to this data, we just need to add the term `(1|ptpt)` to specify that we want a random intercept for each participant:
